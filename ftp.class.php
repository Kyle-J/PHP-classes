...
class ftp
{
    ...
    public function put($remote_filename, $local_file)
    {

        if(file_exists($local_file))
        {
            if (ftp_put($this->conn, $remote_filename, $local_file, FTP_ASCII))
            {
                self::log("Successfully uploaded $local_file to $remote_filename.");

                return true;
            }
            else
            {
                self::log("Error uploading $local_file to $remote_filename.");
                throw new FTPException("Error uploading $local_file to $remote_filename.");
        ...
        {
            self::log('Local file does not exist ' . $local_file);
            throw new FTPException('Local file does not exist ' . $local_file);
    ...
    private static function log($message, $timestamp = true)
    {
        ...
        if (self::$logging)
        {
            ...
            $log_path = dirname(__FILE__) . '/../../../logs/ftp_log';
